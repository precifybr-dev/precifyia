...
export default function MenuMirror() {
  ...
  return (
    <div className="min-h-0 bg-background">
        ...
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          {hasUrl && (
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => fetchMenu(undefined, true)} disabled={isLoading} className="gap-1.5">
                <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? "animate-spin" : ""}`} />
                Atualizar
              </Button>
              <Button variant="ghost" size="sm" onClick={() => setShowUrlInput(true)} className="gap-1.5 text-muted-foreground">
        ...
        <div className="max-w-2xl mx-auto p-4">
          <Card className="border-dashed border-2 border-primary/30">
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex gap-2">
                <Input value={urlInput} onChange={(e) => setUrlInput(e.target.value)} placeholder="https://www.ifood.com.br/delivery/..." className="flex-1" onKeyDown={(e) => e.key === "Enter" && handleSaveUrl()} />
                <Button onClick={handleSaveUrl} disabled={isSaving || !urlInput.trim()}>
                  {isSaving ? "Salvando..." : "Conectar"}
                </Button>
              </div>
              {showUrlInput && (
                <div className="flex justify-between">
                  <Button variant="ghost" size="sm" onClick={() => setShowUrlInput(false)}>Cancelar</Button>
                  <Button variant="ghost" size="sm" onClick={() => { clearUrl(); setShowUrlInput(false); }} className="text-destructive gap-1.5">
      ...
      {hasUrl && !showUrlInput && (
        <MenuPerformanceDashboard analysis={analysis} isLoading={isAnalyzing} onAnalyze={analyzeMenu} hasMenu={!!menuData && menuData.items.length > 0} analysisUsage={analysisUsage} menuItems={menuData?.items} />
      )}

      {hasUrl && !showUrlInput && (
        <IfoodMenuView storeName={menuData?.storeName || "Carregando..."} items={menuData?.items || []} isLoading={isLoading} />
      )}
    </div>
  );
}
...
export default function MenuMirror() {
  ...
  return (
    <div className="min-h-0 bg-zinc-50 dark:bg-zinc-950">
      {/* Header */}
        ...
        <div className="max-w-2xl mx-auto p-4">
          <Card className="border-dashed border-2 border-[#EA1D2C]/30">
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex gap-2">
                <Input value={urlInput} onChange={(e) => setUrlInput(e.target.value)} placeholder="https://www.ifood.com.br/delivery/..." className="flex-1" onKeyDown={(e) => e.key === "Enter" && handleSaveUrl()} />
                <Button onClick={handleSaveUrl} disabled={isSaving || !urlInput.trim()} className="bg-[#EA1D2C] hover:bg-[#c9151f] text-white">
                  {isSaving ? "Salvando..." : "Conectar"}
                </Button>
              </div>
              {showUrlInput && (
