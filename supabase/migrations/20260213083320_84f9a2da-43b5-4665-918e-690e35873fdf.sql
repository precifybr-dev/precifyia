...
-- Atomic usage check + increment function
-- Uses pg_advisory_xact_lock to prevent race conditions on plan limits
...
BEGIN
  ...
  -- 5. Count usage this month (atomic under advisory lock)
  ...
  IF v_count >= v_limit THEN
    RETURN QUERY SELECT
      ...
      v_limit::bigint,
      COALESCE(v_plan, 'free')::text,
      format('Limite de %s uso(s) por mês atingido. Faça upgrade para continuar.', v_limit)::text;
    RETURN;
  END IF;

  -- 7. Increment (still inside advisory lock = atomic)
  INSERT INTO strategic_usage_logs (user_id, endpoint, tokens_used)
  VALUES (_user_id, _endpoint, 1);
