...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Type mapping (fixed: arrays now output correctly as type[])
      IF col_rec.data_type = 'ARRAY' THEN
        -- udt_name for arrays is prefixed with underscore (e.g. _app_role, _text)
        -- Convert to proper SQL syntax: remove underscore prefix and append []
        IF left(col_rec.udt_name, 1) = '_' THEN
          result := result || substring(col_rec.udt_name from 2) || '[]';
        ELSE
